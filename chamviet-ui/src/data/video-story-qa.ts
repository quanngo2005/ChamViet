/**
 * Story content and QA list for Voice AI interaction.
 * Maps videoId → StoryConfig so each video stop can have its own story context.
 *
 * This mirrors the data in service/data/story.py for the Python backend.
 */

export interface StoryQA {
  question: string;
  answer: string; // correct answer for cosine matching
}

export interface StoryConfig {
  storyTitle: string;
  childAge: number;
  storyContent: string; // full story text sent to /api/load-content
  qaList: StoryQA[];
}

// ── Story: Lạc Long Quân và Âu Cơ ────────────────────────────────────────────

const STORY_LAC_LONG_QUAN = `Ngày xưa, ở miền đất Lạc Việt có một vị thần tên là Lạc Long Quân. Thần mình rồng, thường ở dưới nước, thỉnh thoảng lên sống trên cạn, sức khoẻ vô địch, có nhiều phép lạ. Bấy giờ, ở vùng núi cao có nàng Âu Cơ xinh đẹp tuyệt trần, nghe vùng đất Lạc Việt có nhiều hoa thơm cỏ lạ bèn tìm đến thăm. Hai người gặp nhau, kết thành vợ chồng.
...
const QA_LAC_LONG_QUAN: StoryQA[] = [
  {
...
// ── Registry: videoId → StoryConfig ───────────────────────────────────────────

export const STORY_REGISTRY: Record<string, StoryConfig> = {
  Mb0RWyh3sqQ: {
    storyTitle: "Lạc Long Quân và Âu Cơ",
    childAge: 6,
    storyContent: STORY_LAC_LONG_QUAN,
    qaList: QA_LAC_LONG_QUAN,
  },
};
