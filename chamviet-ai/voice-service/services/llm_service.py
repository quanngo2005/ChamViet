...
async def classify_intent(question: str, user_text: str, system_prompt: str) -> str:
    ...
    prompt = (
        ...
        "- ANSWER: Bé đang cố gắng trả lời câu hỏi của cô giáo, dù đúng hay sai.\n"
        "- QUESTION: Bé đang hỏi ngược lại cô giáo hoặc thắc mắc về nội dung truyện.\n"
        "- CONFIRM: Bé đồng ý, xác nhận đã hiểu hoặc đã nghe rõ.\n"
        "- CONFUSED: Bé không biết, không nhớ, hoặc xin cô gợi ý.\n\n"
    ...
    confused_keywords = [
        "khong biet", "hong biet", "khong nho", "hong nho", "con chiu",
    ...
    confirm_keywords = [
        ...
        "hieu roi", "da hieu", "con hieu", "be hieu",
    ]
    if any(user_unsigned == kw or user_unsigned.startswith(kw + " ") for kw in confirm_keywords):
        return "CONFIRM"

    question_keywords = ["tai sao", "la gi", "co oi", "vi sao", "nhu the nao", "o dau"]
...
async def evaluate_story_answer(
    ...
    if not user_answer:
        return {
            ...
            "feedback": "Cô chưa nghe rõ câu trả lời của con. Mình nhớ lại nhẹ nhàng nhé: "
            f"{correct_answer}",
            "reason": "empty_answer",
        }
