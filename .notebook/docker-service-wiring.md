...
- `chamviet-api/src/main/java/com/vn/chamviet/chamviet_api/config/WebConfig.java`: owns browser-facing CORS for all backend endpoints via `/**`; `docker-compose.yml` passes `APP_CORS_ALLOWED_ORIGINS` to include the production ChamViet origins plus local Vite origins.
